...
int r = 1;// user will choose between 1 and 0 while operating - 1 for actual running the program, one for test purposes 
...
void setup () {
  ...
  pinMode (buttonCOpen, INPUT);
  pinMode (buttonDClose, INPUT);
  pinMode (buttonA, INPUT);
  pinMode (buttonB, INPUT);
  //  lcd.clear ();
  //  lcd.print ("Program Resetting...");
  //  delay (1000);
  //mode (); uncomment this if you want test mode option
  intro (); // comment this if you want test mode option
}
//                       END OF SETUP
...
void readbutt(){
  ...
  bc = digitalRead (buttonCOpen);
  bd = digitalRead (buttonDClose);
}

...
struct QSerialRxEvent : public QEvent
{
	unsigned char c;
};

class QPHardwareSerial : public Print
{
  private:
	QActive * *_targetAO;
	uint8_t *_signalID;
    ...
    uint8_t _u2x;
  public:
    QPHardwareSerial( QActive * *targetAO, uint8_t *signalID,
      volatile uint8_t *ubrrh, volatile uint8_t *ubrrl,
      volatile uint8_t *ucsra, volatile uint8_t *ucsrb,
      volatile uint8_t *udr,
      uint8_t rxen, uint8_t txen, uint8_t rxcie, uint8_t udre, uint8_t u2x);
    void begin(long baud, QActive *targetAO, uint8_t signalID);
    void end();
